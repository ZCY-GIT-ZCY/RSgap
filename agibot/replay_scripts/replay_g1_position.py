...
class G1ReplayController:
    ...
    def load_episodes(self):
        ...
        num_episodes = self.loader.info.total_episodes
    ...
    def _load_episode(self, episode_idx: int):
        ...
        ep_info = self.loader.get_episode_info(episode_idx)
    ...
    def set_joint_targets(self, state: np.ndarray):
        """设置关节目标位置"""
        positions = self.mapping.state_to_isaaclab_array(state)
        if not self.debug_printed:
            self.mapping.print_debug(state)
            print(f"\n[Debug] 输出位置数组: {positions}")
    ...
    def step(self) -> bool:
        ...
        self.set_joint_targets(frame.observation_state)
    ...
    def run(self):
        ...
        self.mapping.set_robot_joint_names(actual_joint_names)
