...
class MotionLoaderMotor:
    ...
    def __init__(self, motion_file: str, device: torch.device | str, mode: str, robot_name: str) -> None:
        ...
        # Automatically select the correct joint name dictionary based on robot_name
        if "fourior" in robot_name.lower():
            # For fourior robots, use URDF joint names
            self._dof_names = ROBOT_JOINT_NAME_DICT_URDF_FOURIOR[f"{robot_name}_joints"]
            # Try to get body names from ROBOT_BODY_JOINT_NAME_DICT, if not available use empty list
            try:
                self._body_names = ROBOT_JOINT_NAME_DICT_URDF_FOURIOR[f"{robot_name}_links"]
            except KeyError:
                # If links are not defined for fourior, use empty list
                self._body_names = []
        else:
            # For other robots (e.g., h1), use default ROBOT_BODY_JOINT_NAME_DICT
